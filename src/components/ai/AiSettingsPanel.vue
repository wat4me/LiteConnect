...
function cloneSettings(settings?: AiSettings | null): AiSettings {
  ...
  raw.approvalNotifications = raw.approvalNotifications !== false
  raw.historyMaxThreads = normalizeAiHistoryMaxThreads(raw.historyMaxThreads)
  raw.historyMaxMessages = normalizeAiHistoryMaxMessages(raw.historyMaxMessages)
  return raw
}
...
async function saveSettings() {
  ...
  const next: AiSettings = {
    ...
    approvalNotifications: draftSettings.value.approvalNotifications !== false,
    historyMaxThreads: normalizeAiHistoryMaxThreads(draftSettings.value.historyMaxThreads),
    historyMaxMessages: normalizeAiHistoryMaxMessages(draftSettings.value.historyMaxMessages),
  }
...
<template>
  <div class="settings-box">
    ...
    <template v-if="!editingProvider">
      ...
      <div class="permission-box">
        <span class="field-label">{{ t('ai.historyLimits') }}</span>
        <label for="ai-history-max-threads" class="history-limit-field">
          <span>{{ t('ai.historyMaxThreads') }}</span>
          <input id="ai-history-max-threads" v-model.number="draftSettings.historyMaxThreads" class="ui-input ui-input-sm" type="number" :min="MIN_AI_HISTORY_MAX_THREADS" :max="MAX_AI_HISTORY_MAX_THREADS" step="1" @blur="draftSettings.historyMaxThreads = normalizeAiHistoryMaxThreads(draftSettings.historyMaxThreads)" />
        </label>
        <label for="ai-history-max-messages" class="history-limit-field">
          <span>{{ t('ai.historyMaxMessages') }}</span>
          <input id="ai-history-max-messages" v-model.number="draftSettings.historyMaxMessages" class="ui-input ui-input-sm" type="number" :min="MIN_AI_HISTORY_MAX_MESSAGES" :max="MAX_AI_HISTORY_MAX_MESSAGES" step="10" @blur="draftSettings.historyMaxMessages = normalizeAiHistoryMaxMessages(draftSettings.historyMaxMessages)" />
        </label>
        <p class="permission-hint">{{ t('ai.historyLimitsHint') }}</p>
      </div>
      <div class="permission-box">
        <span class="field-label">{{ t('ai.approvalNotifications') }}</span>
        <label class="notification-option">
          <input v-model="draftSettings.approvalNotifications" type="checkbox" />
          <span>
            <span class="permission-option-title">{{ t('ai.approvalNotificationsEnabled') }}</span>
            <span class="permission-option-desc">{{ t('ai.approvalNotificationsHint') }}</span>
          </span>
        </label>
      </div>
      <div class="permission-box">
