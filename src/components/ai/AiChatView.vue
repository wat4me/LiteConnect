...
function messageTimeText(message: ChatItem): string {
  const timestamp = messageDisplayTimestamp(message)
  return timestamp ? formatMessageTime(timestamp) : ''
}

function messageTimeTitle(message: ChatItem): string {
  const timestamp = messageDisplayTimestamp(message)
  if (!timestamp) return ''
  const time = formatMessageTimeDetail(timestamp)
  return message.role === 'user' ? t('ai.sentAt', { time }) : t('ai.replyCompletedAt', { time })
}

function messageTimeIso(message: ChatItem): string {
  const timestamp = messageDisplayTimestamp(message)
...
<template>
  <div class="chat-shell">
    ...
    >
      ...
      >
        <time
          v-if="messageTimeText(message)"
          class="message-time"
          :datetime="messageTimeIso(message)"
          :title="messageTimeTitle(message)"
          :aria-label="messageTimeTitle(message)"
        >{{ messageTimeText(message) }}</time>
...
.markdown-block :deep(.md-image-link) {
  display: inline-flex;
  max-width: 100%;
  overflow-wrap: anywhere;
}

.chat-row.assistant .message-stack {
  gap: 4px;
}
...
.reasoning-pending .reasoning-summary {
  cursor: default;
  pointer-events: none;
}
...
.chat-row:hover .message-time,
.chat-row:focus-within .message-time,
.chat-row:hover .usage-line,
.chat-row:focus-within .usage-line {
  opacity: 0.72;
  visibility: visible;
}
