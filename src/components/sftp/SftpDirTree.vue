...
/** Selection keyed by path and holding the entry, so it survives collapsing / searching / paging. */
const selectedEntries = ref<Map<string, FileEntry>>(new Map())
const selectedPaths = computed(() => new Set(selectedEntries.value.keys()))
...
const selectedFileEntries = computed(() => [...selectedEntries.value.values()])
...
function toggleSelect(entry: FileEntry, e?: MouseEvent) {
  ...
  if (e?.shiftKey && lastClickedPath) {
    ...
    if (a >= 0 && b >= 0) {
      ...
      for (let i = from; i <= to; i++) next.set(list[i].path, list[i].entry)
...
/** Drop selected files that a fresh listing of their (expanded) directory no longer contains. */
function pruneStaleSelection() {
  ...
  for (const p of next.keys()) {
    ...
    if (!props.isExpanded(parent) || props.isLoading(parent)) continue
    if (!props.entriesOf(parent).some((e) => e.path === p)) {
      next.delete(p)
      changed = true
    }
  }
  if (changed) selectedEntries.value = next
}
