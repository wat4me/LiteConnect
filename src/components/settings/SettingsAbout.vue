...
const statusText = computed(() => {
  ...
  if (st.status === 'cancelled') return t('about.downloadCancelled')
  if (st.status === 'error') return t('about.error', { message: st.message || t('about.checkFailed') })
  return ''
})

/** 下载中不显示重试按钮，其余「有版本但没下载完」的情况都允许重新发起。 */
const canDownload = computed(() => {
  const st = updateStatus.value
  if (!st?.version) return false
  return st.status === 'available' || st.status === 'cancelled' || st.status === 'error'
})
...
async function cancelDownload() {
  ...
  try {
    const result = await window.LiteConnect.cancelUpdateDownload()
    if (!result.ok) {
      updateStatus.value = {
        status: 'error',
