...
export function useDbQueryDrafts(deps: DbQueryDraftsDeps) {
  ...
  async function loadDraftsFromStorage() {
    try {
      const raw = await window.LiteConnect.getRendererState('db-query-drafts')
  ...
  async function writeDraftsToStorage() {
    try {
      ...
      // Keep the current in-memory drafts and retry after the next edit.
    }
  }
  ...
  async function initQueryDrafts() {
    await loadDraftsFromStorage()
    const known = new Set(deps.connections.value.map((c) => c.id))
    draftRecords = pruneOrphanDrafts(draftRecords, known)
    await writeDraftsToStorage()
  }
