...
async function loadHistory(): Promise<BatchHistoryItem[]> {
  try {
    const raw = await window.LiteConnect.getRendererState('batch-command-history')
    if (!raw) return []
...
async function saveHistory(items: BatchHistoryItem[]) {
  await window.LiteConnect.setRendererState(
...
export function useBatchCommand() {
  ...
  const command = ref('')
  const recentHistory = ref<BatchHistoryItem[]>([])
  let runToken = 0
  ...
  function pushHistory(entry: Omit<BatchHistoryItem, 'id' | 'at'>) {
    ...
    void saveHistory(recentHistory.value).catch(() => {})
  }
