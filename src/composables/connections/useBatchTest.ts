...
import { t } from '@/i18n'
import { sshTestErrorKey } from '@/utils/connections/sshTestError'
  ...
  async function onTestConnection(connectionId: string) {
    ...
    try {
      ...
      if (result.ok) {
        ...
        const errorKey = sshTestErrorKey(result.error, result.stage)
        let error = errorKey ? t(errorKey) : result.error || t('connections.connectFailed')
      ...
      testStatuses.value.set(connectionId, { state: 'error', error: t('connections.testFailed') })
      const timer = setTimeout(() => {
        if (testStatuses.value.get(connectionId)?.state === 'error') {
          testStatuses.value.delete(connectionId)
        }
