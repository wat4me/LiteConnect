...
export function useSftpDirTree(sessionId: () => string) {
  ...
  async function followPath(
    targetPath: string,
    opts?: { forceAncestors?: boolean; injectPlaceholder?: boolean },
  ): Promise<void> {
    ...
    // and scroll-to-path / highlight can work. Callers that know the leaf may
    // be gone (refresh after a failed readdir) opt out so no phantom row appears.
    if (chain.length >= 2 && opts?.injectPlaceholder !== false) {
  ...
  async function refreshPathChain(targetPath: string, opts?: { injectPlaceholder?: boolean }): Promise<void> {
    await followPath(targetPath, { forceAncestors: true, injectPlaceholder: opts?.injectPlaceholder })
