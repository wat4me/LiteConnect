...
export function useSftpFileActions(deps: {
  ...
  async function confirmRename() {
    ...
    // A slash would turn the rename into a move; dot names and NUL are never valid entry names.
    // Toast rather than the sidebar banner: the rename dialog is still open and would cover it.
    if (newName.includes('/') || newName.includes('\0') || newName === '.' || newName === '..') {
      ElMessage.warning(t('sftp.invalidName'))
      return
    }
