...
export type TransferFinishStatus = 'completed' | 'error' | 'skipped' | 'partial' | 'cancelled'
...
function noteTransferFinished(
  ...
  const batch = batchId ? transferBatches.get(batchId) : undefined
  if (!batch) {
  ...
  transferBatches.delete(batch.id)
}
...
/** Cancelled (by the user or by session teardown): resumable error state, no failure toast. */
function markCancelled(sessionId: string, transferId: string) {
  markError(sessionId, transferId, t('sftp.cancelledResumable'))
}
...
export function ensureTransferListeners() {
  ...
  unsubFns = [
    window.LiteConnect.onTransferStart((sessionId, transferId, fileName, localPath, direction, remotePath, kind) => {
      addTransfer(sessionId, transferId, fileName, localPath, direction, remotePath, kind)
    }),
    ...
    window.LiteConnect.onTransferError((sessionId, transferId, errorMsg, code) => {
      const item = transfers.get(transferId)
      if (code === 'cancelled') {
        markCancelled(sessionId, transferId)
        noteTransferFinished(sessionId, transferId, 'cancelled', {
          fileName: item?.fileName || '',
          direction: item?.direction || 'download',
        })
        return
...
/** Single-file transfers only; a directory job has no one stream to pick up again. */
export function canResumeTransfer(item: TransferItem): boolean {
  return item.status === 'error' && item.kind !== 'directory' && !!item.localPath && !!item.remotePath
}
...
export function useTransfers(getCurrentSessionId: () => string) {
  ...
  /** Optimistic local state; main confirms with a `cancelled` transferError once its streams are down. */
  function cancelTransfer(transferId: string) {
    window.LiteConnect.sftpCancelTransfer(transferId)
    const item = transfers.get(transferId)
    if (item) markCancelled(item.sessionId, transferId)
  }
  ...
  function resumeTransfer(transferId: string) {
    const item = transfers.get(transferId)
    if (!item || !canResumeTransfer(item)) return false
    ...
    if (item.direction === 'download') {
      ...
      window.LiteConnect.sftpUpload(
        ...
        item.fileName,
        newId,
        { resume: true, remoteFullPath: remotePath },
      )
    }
    return true
  }
