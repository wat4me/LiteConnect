 ...
 *
 * Every explicit file/directory remains one upload item. Inferring a directory
 * from several sibling files is unsafe: the same shape is produced when a user
 * deliberately selects only a few files, and uploading their parent would also
 * send every unselected sibling.
 *
...
export function expandDroppedFiles(files: DropPathInfo[]): DropUploadItem[] {
  ...
  const trusted = files.filter((f) => {
    if (f.isDirectory) return isSynthesizableUploadRoot(f.path)
