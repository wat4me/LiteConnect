export default {
  settingsMcp: {
    ...
    policyDetails: '查看命令拦截规则',
    policyDetailsHint:
      '这里只展示 exec / service_control 的命令分类规则。分类器会解析整条命令，管道、重定向、参数和子命令都可能改变最终结果；下面的二进制名单不是简单的字符串屏蔽。',
    policyItems: '{count} 项',
    policyCurrentDeny: '当前模式：直接拒绝',
    policyCurrentDenyHint:
      '危险、提权和高危命令会返回 DENIED。客户端传 confirmed=true 也不能覆盖；需要先把命令权限改为「在 MCP 客户端询问」或「自动执行」。',
    policyCurrentAsk: '当前模式：先询问',
    policyCurrentAskHint:
      '危险和提权命令先返回 APPROVAL_REQUIRED；你在 MCP 客户端确认后，同一命令带 confirmed=true 可执行。高危命令仍需切换为「自动执行」。',
    policyCurrentAuto: '当前模式：自动执行',
    policyCurrentAutoHint: '所有分类均直接执行，包括删除、提权和高危组合；远端账号本身的权限仍然有效。',
    policyDestructive: '危险命令',
    policyDestructiveHint:
      '这些程序默认视为会删除数据、改变权限、进程、服务或系统配置。个别明确的只读参数会先被识别，例如 kill -0。',
    policyPrivileged: '提权命令',
    policyPrivilegedHint: '以下提权包装器单独归类，行为由当前命令权限决定。',
    policyHighRisk: '高危命令组合',
    policyHighRiskHint: '系统目录递归删除、裸盘写入、下载后直接执行、关键认证文件写入等会被提升为最高风险。',
    policyDynamic: '按参数动态判定',
    policyDynamicHint: '同一个程序可能是查询，也可能会修改系统；最终以完整命令的解析结果为准。',
    policyRuleKillProbe: '仅检查进程是否存在，按只读处理；其他信号按危险命令处理。',
    policyRuleSystemctl: 'status/show 等查询为只读；启动、停止、重启、启用等操作为危险。',
    policyRuleGit: '查询通常为只读，普通修改为安全写入；清理、硬重置、强制推送为危险。',
    policyRuleMutationFlags: '出现原地修改、删除、执行或同步删除参数时按危险命令处理。',
    policyRuleNetwork: '只向标准输出读取通常为只读；上传、非只读请求或写入本地文件按写入处理。',
    policyRuleCli: '会识别常见查询子命令；修改资源、运行容器或应用基础设施变更时提高风险。',
    policyRuleScripts: '能展开并检查的 shell 代码继续逐条判定；无法检查的脚本或内联解释器代码按危险处理。',
    policyRuleInterpreters: '脚本内容无法可靠检查时按危险命令处理。',
    policyUnknownCommand: '未收录的程序 / 动态命令名',
    policyRuleUnknown: '采用保守策略，按危险命令处理并要求人工决定。',
    anyClient: 'MCP 客户端接入',
    anyClientHint:
