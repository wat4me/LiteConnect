...
import { abortAiChatStream, createAiStreamControl, resolveToolApproval, runAiChatStream } from './chatStream'
...
it('only lets the owning renderer abort a stream', () => {
  const control = createAiStreamControl('owned-request', 7)
  expect(abortAiChatStream('owned-request', 8)).toBe(false)
  expect(control.controller.signal.aborted).toBe(false)
  expect(abortAiChatStream('owned-request', 7)).toBe(true)
  expect(control.controller.signal.aborted).toBe(true)
  control.cleanup()
})

it('does not retry authentication failures just to remove stream_options', async () => {
  const fetchMock = vi.fn().mockResolvedValue(new Response('unauthorized', { status: 401 }))
  vi.stubGlobal('fetch', fetchMock)
  await expect(runAiChatStream({
    emit: () => {}, requestId: 'no-auth-retry', settings,
...
it.each([true, false])('honors declared write approval (%s) and retains the explanation', async approved => {
  ...
  const closeApprovalNotification = vi.fn()
  const onToolApprovalRequested = vi.fn(() => closeApprovalNotification)
  ...
  expect(onToolApprovalRequested).toHaveBeenCalledWith({
    requestId,
    callId: 't',
