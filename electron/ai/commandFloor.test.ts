...
describe('commandContentRisk', () => {
  ...
  it('allows common read-only performance monitors, including filtered top output', () => {
    const topSnapshot = `top -bn2 -o %CPU | awk '/^top -/{n++} n==2' | head -n 20`
    expect(classOf('exec', { command: topSnapshot })).toBe('read-only')
    expect(gateFor(topSnapshot, 'read', 'ask').action).toBe('allow')
    for (const command of ['vmstat 1 3', 'mpstat 1 3', 'iostat -xz 1 2', 'pidstat 1 2', 'sar -u 1 2']) {
      expect(classOf('exec', { command })).toBe('read-only')
      expect(gateFor(command, 'read', 'ask').action).toBe('allow')
    }
  })
