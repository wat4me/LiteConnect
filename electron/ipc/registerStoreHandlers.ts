...
function validateRendererStateKey(key: unknown): string {
  if (typeof key !== 'string' || !RENDERER_STATE_KEYS.has(key)) {
    throw new Error('Invalid renderer state key')
  }
  return key
}

function validateRendererStateValue(value: unknown): string | null {
  if (value === null) return null
  if (typeof value !== 'string' || value.length > RENDERER_STATE_MAX_CHARS) {
    throw new Error('Invalid renderer state value')
  }
  return value
}
...
export function registerStoreHandlers(
  ...
  ipcMain.handle('store:getRendererState', (_event, key: unknown) => {
    const validKey = validateRendererStateKey(key)
    return getAppDatabase().getSingleton<string>(rendererStateKey(validKey)) ?? null
  })

  ipcMain.handle('store:setRendererState', (_event, key: unknown, value: unknown) => {
    const validKey = validateRendererStateKey(key)
    const validValue = validateRendererStateValue(value)
    const database = getAppDatabase()
    if (validValue === null) database.deleteSingleton(rendererStateKey(validKey))
    else database.setSingleton(rendererStateKey(validKey), validValue)
  })

  ipcMain.handle('store:migrateRendererState', (_event, entries: unknown) => {
    if (!entries || typeof entries !== 'object' || Array.isArray(entries)) {
      throw new Error('Invalid renderer state migration')
    }
    const database = getAppDatabase()
    database.transaction(() => {
      for (const [key, value] of Object.entries(entries)) {
        const validKey = validateRendererStateKey(key)
        const validValue = validateRendererStateValue(value)
        const storageKey = rendererStateKey(validKey)
        if (validValue !== null && !database.hasSingleton(storageKey)) {
          database.setSingleton(storageKey, validValue)
        }
      }
    })
  })
