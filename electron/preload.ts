...
contextBridge.exposeInMainWorld('LiteConnect', {
  ...
  onAiApprovalNotificationClick: (callback: (sessionId: string) => void) => {
    const listener = (_event: any, sessionId: string) => callback(sessionId)
    ipcRenderer.on('ai:approvalNotificationClick', listener)
    return () => ipcRenderer.removeListener('ai:approvalNotificationClick', listener)
  },
  ...
  onTransferStart: (
    ...
    const listener = (
      ...
      remotePath?: string,
      kind?: 'file' | 'directory',
    ) => callback(sessionId, transferId, fileName, localPath, direction as 'download' | 'upload', remotePath, kind)
  ...
  onTransferError: (
    callback: (sessionId: string, transferId: string, error: string, code?: 'cancelled') => void,
  ) => {
    const listener = (_event: any, sessionId: string, transferId: string, error: string, code?: 'cancelled') =>
      callback(sessionId, transferId, error, code)
  ...
  getAppResourceStats: () => ipcRenderer.invoke('app:getResourceStats'),
