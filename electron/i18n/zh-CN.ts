...
export default {
  ...
  sftp: {
    ...
    notUtf8Text: '文件不是 UTF-8 文本（可能是二进制或其它编码），内置编辑器无法安全编辑',
  ...
  ai: {
    approvalNotificationTitle: 'LiteConnect · AI 等待审批',
    approvalNotificationBody: 'AI 请求执行 {tool}，请返回 LiteConnect 查看并确认。',
    ...
    markdownOnly: '只支持 .md 或 .markdown 文件',
    markdownSizeLimit: 'Markdown 文件不能超过 32 KB',
    markdownEmpty: '请选择非空的 Markdown 文本文件',
    contextFileLimit: '最多支持 {count} 份参考文件，请先移除一份。',
  ...
  updateGuard: {
    title: '更新正在下载',
    message: '正在下载 {version}（{progress}%），现在退出会中断下载。',
    detail: '已下载的部分不会保留，下次需要重新下载。',
    stay: '继续下载',
    quit: '仍然退出',
  },
} as const
