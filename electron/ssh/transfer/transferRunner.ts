...
import { claimLocalName, joinRemote, nextRemoteName, remoteBasename, remoteDirname, sanitizeLocalName } from '../../utils/filePaths'
import type { FileEntry } from '../types'
import { t } from '../../i18n'
import { sftpDownload as runSftpDownload, sftpUpload as runSftpUpload } from './fileTransfer'
import {
...
export class TransferRunner {
  ...
  /** True when the id (or a directory job owning it) was active; false means nothing here knew it. */
  ...
  private fileTransferDeps() {
    return {
      getSession: this.getSession,
      activeTransfers: this.activeTransfers,
      destroyStreams: (transfer: ActiveTransfer) => this.destroyStreams(transfer),
    }
  }
  ...
  sftpDownload(
    ...
    return runSftpDownload(this.fileTransferDeps(), sessionId, remotePath, localPath, transferId, onProgress, options)
  }
  ...
  sftpUpload(
    ...
    return runSftpUpload(this.fileTransferDeps(), sessionId, localPath, remotePath, transferId, onProgress, options)
  }
  ...
  private async walkRemoteTree(
    ...
    while (queue.length > 0) {
      ...
      for (const e of list) {
        ...
        if (!e.name || e.name.includes('\0') || e.name.includes('/')) {
          throw new Error(`Unsafe remote filename: ${JSON.stringify(e.name)}`)
        }
        // Remote names may be illegal locally (Windows `:`, `?`, …); rewrite rather than abort the tree.
        const localName = claimLocalName(usedLocalNames, sanitizeLocalName(e.name))
        const lChild = path.resolve(lDir, localName)
        const relativeToRoot = path.relative(path.resolve(localRoot), lChild)
        if (
          relativeToRoot === '..' ||
          relativeToRoot.startsWith(`..${path.sep}`) ||
          path.isAbsolute(relativeToRoot)
        ) {
          throw new Error(`Remote filename escapes download directory: ${JSON.stringify(e.name)}`)
        }
        if (e.isDirectory) {
