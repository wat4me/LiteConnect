...
/** Raw result of a remote command; stdout and stderr are kept apart. */
...
export function execFailureMessage(out: ExecOutput): string {
  const detail = out.stderr.trim() || out.stdout.trim()
  if (detail) return detail
  if (out.signal) return `Command terminated by signal ${out.signal}`
  return `Command exited with code ${out.code}`
}

/** Strict UTF-8 decode; null when the bytes are not valid UTF-8 or contain NUL (binary). */
function decodeUtf8Text(buffer: Buffer): string | null {
  if (buffer.includes(0)) return null
  try {
    // Keep a BOM in the text so saving writes it back unchanged.
...
export class SftpSession {
  ...
  async initSftp(sessionId: string): Promise<void> {
    ...
    const promise = new Promise<void>((resolve, reject) => {
      session.client.sftp((err, sftp) => {
        if (err) {
          ...
          session.sftp = sftp
          this.watchSftpChannel(session, sftp)
          resolve()
        }
      })
    })
    ...
    try {
      ...
      // The ssh2 callback is normally asynchronous, but test doubles and other
      // clients may invoke it synchronously. Clear only after the promise has
      // been registered, and never delete a newer in-flight initialization.
      if (this.sftpInitPromises.get(sessionId) === promise) {
        this.sftpInitPromises.delete(sessionId)
      }
    }
  }

  /** Drop the channel reference once it dies so the next call re-opens it instead of failing forever. */
  private watchSftpChannel(session: Session, sftp: SFTPWrapper) {
    const forget = () => {
  ...
  async sftpExtractArchive(
    ...
    const out = await this.execRaw(sessionId, cmd, timeoutMs)
    // Warnings after a complete extraction: unzip / 7z exit 1, gzip exits 2 (1 is a real error).
    const tolerated = kind === 'zip' || kind === '7z' ? [0, 1] : kind === 'gz' ? [0, 2] : [0]
    if (out.signal || (out.code !== null && !tolerated.includes(out.code))) {
      throw new Error(execFailureMessage(out))
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const finish = (err?: Error) => {
        ...
        if (err) {
          ...
          const text = decodeUtf8Text(Buffer.concat(chunks))
          if (text === null) reject(new Error(t('sftp.notUtf8Text')))
          else resolve(text)
        }
      }
      stream.on('data', (chunk: Buffer) => {
  ...
  async sftpWriteBuffer(
    ...
    // TODO: Atomic temp+rename writes are deferred: standard SFTP rename may not
    // replace an existing target and can change symlink/owner/hard-link semantics.
    const session = this.getSession(sessionId)
    if (!session?.sftp) throw new Error(t('sftp.notInitialized'))
  ...
  async sftpChmod(sessionId: string, remotePath: string, mode: string, recursive = false): Promise<void> {
    ...
    await this.sftpExec(sessionId, `chmod ${flag}${mode} -- ${escapedPath}`, 60000)
  }
  ...
  async sftpChown(
    ...
    const flag = recursive ? '-R ' : ''
    const escapedPath = shellQuote(remotePath)
    await this.sftpExec(sessionId, `chown ${flag}${spec} -- ${escapedPath}`, 60000)
  }
  ...
  /** Run a remote command; stdout and stderr stay separate so callers can judge the exit themselves. */
  execRaw(sessionId: string, command: string, timeoutMs = 10000): Promise<ExecOutput> {
    const session = this.getSession(sessionId)
    ...
    return new Promise((resolve, reject) => {
      ...
      const noteExit = (exitCode?: number | null, exitSignal?: string | null) => {
        if (typeof exitCode === 'number' && Number.isFinite(exitCode)) code = exitCode
        if (typeof exitSignal === 'string' && exitSignal) signal = exitSignal
      }
      ...
      session.client.exec(command, (err, stream) => {
        ...
        stream.on('exit', noteExit)
        stream.on('close', (exitCode?: number | null, exitSignal?: string | null) => {
          noteExit(exitCode, exitSignal)
          finish()
        })
  ...
  /** Convenience over execRaw: non-zero exit is an error, success returns stdout only. */
  async sftpExec(sessionId: string, command: string, timeoutMs = 10000): Promise<string> {
    const out = await this.execRaw(sessionId, command, timeoutMs)
