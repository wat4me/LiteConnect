...
import { SftpSession, type ExecOutput } from './sftp/sftpSession'
...
export class SSHManager {
  ...
  /** False when no active transfer (or directory job) knew the id. */
  ...
  /** Raw exec (stdout, stderr, exit status) for callers that must show everything, e.g. batch commands. */
  execRaw(sessionId: string, command: string, timeoutMs = 10000): Promise<ExecOutput> {
    return this.sftp.execRaw(sessionId, command, timeoutMs)
  }
