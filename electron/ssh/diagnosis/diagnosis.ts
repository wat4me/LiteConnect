...
export async function diagnoseSshConnection(
  ...
  try {
    tcpLatency = await testTcpLatency(tcpHost, tcpPort, timeoutMs)
  } catch (err: any) {
    return {
      ok: false,
      tcpLatency,
  ...
  return new Promise((resolve) => {
    ...
    const finish = (result: SshDiagnosisResult) => {
      ...
      disposeSshChannel(stream)
      disposeSshClient(client)
      disposeSshClient(jumpClient)
      resolve({
        tcpLatency,
        sshReadyLatency,
        shellOpenLatency,
    ...
    shellTimeout = setTimeout(() => {
      finish({
        ...
        stage: 'ssh_handshake',
        error: `SSH diagnosis timeout (${timeoutMs}ms)`,
      })
    }, timeoutMs)
