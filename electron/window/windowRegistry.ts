...
export function getWindowRole(win: BrowserWindow): 'main' | 'database' | 'detached' {
  if (getDbWindow() === win) return 'database'
  for (const detached of detachedByConnection.values()) {
    if (detached === win) return 'detached'
  }
  return 'main'
}
