import { existsSync } from 'fs'
import { join } from 'path'

export type ConflictStrategy = 'overwrite' | 'skip' | 'rename'

/** Windows forbids these in file names; NTFS also treats `name:stream` as an alternate data stream. */
const WINDOWS_ILLEGAL_CHARS = /[<>:"/\\|?*\x00-\x1f]/g
const WINDOWS_RESERVED_NAMES = /^(con|prn|aux|nul|com[1-9]|lpt[1-9])(\.|$)/i

/**
 * Make a remote entry name safe to create locally. Only Windows needs rewriting;
 * elsewhere a name that came from readdir cannot contain `/` or NUL anyway.
 */
export function sanitizeLocalName(name: string, platform: NodeJS.Platform = process.platform): string {
  if (platform !== 'win32') return name
  let out = name.replace(WINDOWS_ILLEGAL_CHARS, '_')
  // Windows silently strips trailing dots and spaces, which would merge distinct names.
  out = out.replace(/[. ]+$/, '')
  if (WINDOWS_RESERVED_NAMES.test(out)) out = `_${out}`
  return out || '_'
}

function splitExtension(filename: string, isDirectory: boolean): { baseName: string; ext: string } {
  if (isDirectory) return { baseName: filename, ext: '' }
  const ext = filename.includes('.') ? '.' + filename.split('.').pop() : ''
  return { baseName: ext ? filename.slice(0, -ext.length) : filename, ext }
}

export function getUniqueLocalPath(dir: string, filename: string, isDirectory = false): string {
  let filePath = join(dir, filename)
  if (!existsSync(filePath)) return filePath

  const { baseName, ext } = splitExtension(filename, isDirectory)
  let counter = 1
  while (existsSync(join(dir, `${baseName} (${counter})${ext}`))) {
    counter++
  }
  return join(dir, `${baseName} (${counter})${ext}`)
}

/** Resolve local destination path given conflict strategy. Returns null when skip. */
export function resolveLocalConflictPath(
  dir: string,
  filename: string,
  strategy: ConflictStrategy,
  opts?: { isDirectory?: boolean },
): string | null {
  const target = join(dir, filename)
  if (!existsSync(target)) return target
  if (strategy === 'overwrite') return target
  if (strategy === 'skip') return null
  return getUniqueLocalPath(dir, filename, opts?.isDirectory === true)
}

/** Generate unique remote file name (basename only) when target exists. */
export function nextRemoteName(existingNames: Set<string>, filename: string): string {
  if (!existingNames.has(filename)) return filename
  const { baseName, ext } = splitExtension(filename, false)
  let counter = 1
  while (existingNames.has(`${baseName} (${counter})${ext}`)) {
    counter++
  }
  return `${baseName} (${counter})${ext}`
}

/** Unique-enough remote name when the target exists but its directory could not be listed. */
export function fallbackRemoteName(filename: string, now = Date.now()): string {
  const { baseName, ext } = splitExtension(filename, false)
  return `${baseName} (${now.toString(36)})${ext}`
}

/**
 * Pick a local name not yet in `used` and record it. Folds case where the local
 * filesystem does, so remote `A.txt` and `a.txt` never land on the same file.
 */
export function claimLocalName(
  used: Set<string>,
  name: string,
  foldCase = process.platform === 'win32' || process.platform === 'darwin',
): string {
  const key = (n: string) => (foldCase ? n.toLowerCase() : n)
  const { baseName, ext } = splitExtension(name, false)
  let candidate = name
  let counter = 1
  while (used.has(key(candidate))) {
    candidate = `${baseName} (${counter})${ext}`
    counter++
  }
  used.add(key(candidate))
  return candidate
}

export function joinRemote(parent: string, name: string): string {
  if (!parent || parent === '/') return `/${name}`
  return parent.endsWith('/') ? `${parent}${name}` : `${parent}/${name}`
}

export function remoteDirname(remotePath: string): string {
  const cleaned = remotePath.replace(/\/+$/, '') || '/'
  const idx = cleaned.lastIndexOf('/')
  if (idx <= 0) return '/'
  return cleaned.slice(0, idx) || '/'
}

export function remoteBasename(remotePath: string): string {
  const cleaned = remotePath.replace(/\/+$/, '') || '/'
  const idx = cleaned.lastIndexOf('/')
  return idx >= 0 ? cleaned.slice(idx + 1) : cleaned
}

/** Detect archive type from filename for remote extract. */
export type ArchiveKind = 'tar' | 'targz' | 'tarbz2' | 'tarxz' | 'zip' | 'gz' | '7z'

export function detectArchiveKind(fileName: string): ArchiveKind | null {
  const lower = fileName.toLowerCase()
  if (lower.endsWith('.tar.gz') || lower.endsWith('.tgz')) return 'targz'
  if (lower.endsWith('.tar.bz2') || lower.endsWith('.tbz2') || lower.endsWith('.tbz')) return 'tarbz2'
  if (lower.endsWith('.tar.xz') || lower.endsWith('.txz')) return 'tarxz'
  if (lower.endsWith('.tar')) return 'tar'
  if (lower.endsWith('.zip')) return 'zip'
  if (lower.endsWith('.7z')) return '7z'
  if (lower.endsWith('.gz') && !lower.endsWith('.tar.gz')) return 'gz'
  return null
}

export function buildExtractCommand(remotePath: string, kind: ArchiveKind): string {
  // Paths are shell-quoted by caller via shellQuote in manager
  switch (kind) {
    case 'targz':
      return `tar -xzf`
    case 'tarbz2':
      return `tar -xjf`
    case 'tarxz':
      return `tar -xJf`
    case 'tar':
      return `tar -xf`
    case 'zip':
      return `unzip -o`
    case '7z':
      return `7z x -y`
    case 'gz':
      return `gunzip -k -f`
    default:
      return `tar -xf`
  }
}
