...
- 保存、分组、排序和测试 SSH 连接
- 支持密码、私钥和系统 SSH Agent 认证
- 支持跳板机、本地端口转发和 X11 转发
- 使用本地 `known_hosts` 记录主机密钥；首次连接需要确认，密钥变化时拒绝连接
...
- SSH 断开或应用退出时关闭 Docker 代理、日志流和远端通道

当前 Docker 模块不包含删除容器、镜像管理、Volume 管理、Compose 或容器交互终端。
...
│   ├── store/                   # 连接、凭据、设置和查询历史存储
│   ├── utils/                   # 主进程通用工具
│   ├── window/                  # BrowserWindow 创建与配置
│   ├── main.ts                  # 主进程入口和退出清理
│   └── preload.ts               # 暴露给 renderer 的受控 API
├── shared/                      # main 与 renderer 共用的纯逻辑
├── src/                         # Vue renderer
│   ├── components/              # 终端、SFTP、Docker、数据库和通用组件
│   │   ├── database/            # 数据库工作区组件
│   │   └── icons/               # 应用图标组件
│   ├── composables/             # 会话和 UI 状态逻辑
│   │   └── database/            # 数据库 renderer 状态逻辑
...
- SSH、数据库和 Docker 的 socket、stream、日志及连接资源由主进程持有。
...
| 文件或目录 | 内容 |
|---|---|
| `connections.json` / `groups.json` | SSH 连接和分组 |
