...
- Top 进程
- 主机名、内核、架构和运行时长
- 可为每台已连接的服务器单独开启 CPU、内存资源告警，设置 50–99% 阈值和连续 24 小时最多 1–10 次系统通知。连续 3 次采样超限才提醒，持续超限至少间隔 30 分钟；点击通知可回到对应服务器监控。

监控数据通过 SSH 执行系统命令采集，具体可用字段取决于远端操作系统和命令环境。告警规则在监控面板的「资源告警」中设置，默认关闭；启用后关闭面板仍会继续监控，SSH 断开或 LiteConnect 退出后停止。关闭到托盘时应用仍在运行。
