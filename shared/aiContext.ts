...
export function sliceToTokens(text: string, maxTokens: number, fromEnd = false): string {
  ...
  let cjk = 0
  let other = 0
  const tokenCount = () => cjk + Math.ceil(other / 4)
  const start = fromEnd ? chars.length - 1 : 0
  const end = fromEnd ? -1 : chars.length
  const step = fromEnd ? -1 : 1
  let boundary = fromEnd ? chars.length : 0
  for (let i = start; i !== end; i += step) {
    const code = chars[i].codePointAt(0) || 0
    if (isCjkCodePoint(code)) cjk += 1
    else other += 1
    if (tokenCount() > maxTokens) break
    boundary = fromEnd ? i : i + 1
  }
  return fromEnd ? chars.slice(boundary).join('') : chars.slice(0, boundary).join('')
}
